...
fn init_from_expr(expr: &Expr) -> (usize, TokenStream) {
    match expr {
...
fn mutself_impl(input: MacroInput) -> TokenStream {
    ...
    let const_decl = input.entries.iter().enumerate().map(|(i, e)| {
        let ty_name = &e.name;
        ...
        quote! {
            ...
            #vis const #ty_name: #ty_name = #ty_name { __priv: () };
        }
    });
    ...
    quote! {
        mod __mutself {
            ...
            #[inline(always)]
            pub(super) fn item<const ITEM: usize>() -> &'static [u8] {
        ...
        pub fn mutself<P: AsRef<Path>>(
            ...
            use object::pe::{
                ...
                IMAGE_DIRECTORY_ENTRY_EXCEPTION, IMAGE_SCN_CNT_INITIALIZED_DATA, IMAGE_SCN_MEM_READ
            };
